...
const validateFormList = (form, fields, name, reValidate) => {
  // console.log('test 1');
...
const onValuesChangeForm = (form, fields) => {
  ...
  fields.reduce((result, item, index) => {
    console.log('ward', form.getFieldValue(['travel', index, 'ward']));
...
const FormTest = ({ name, form, remove, fields, isListField }) => {
  ...
  const handleCityChange = (value, option, name) => {
    ...
    validateFormList(
      form,
      form.getFieldValue('travel'),
      currentDistrict ? name : undefined,
      true,
    );
  };
